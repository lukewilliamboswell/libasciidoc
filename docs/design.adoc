...
The DOCX renderer (`pkg/renderer/docx/`) takes the final document and writes an OpenXML package directly using the Go standard library. Unlike the SGML-based HTML/XHTML renderers which use Go text templates, the DOCX renderer builds the required ZIP parts (`word/document.xml`, relationships, styles, numbering, footnotes, and media) and writes WordprocessingML paragraphs, runs, tables, images, links, bookmarks, and footnote references directly.
